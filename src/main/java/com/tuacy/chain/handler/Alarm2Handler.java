 ...
 * @date 2020/6/25 1:41
 */
@Handler(group = "alarm", order = 2)
public class Alarm2Handler implements IHandler<List<AlarmInfo>> {

    private String name = "AlarmPush2Handler";

    @Override
    public void handlerProcess(HandlerContext ctx, List<AlarmInfo> source) {
        System.out.println("AlarmPush2Handler");
    }
}
